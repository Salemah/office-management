...
@section('title', 'Purchase Payment History')
            ...
            <div class="modal-body modal-edit">
                <form class="fromClass" action="#" enctype="multipart/form-data" method="POST" >
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                            <label for="received_amount"><b>Payable Amount</b><span class="text-danger">*</span></label>
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                            <label for="paying_amount"><b>Paid Amount</b><span class="text-danger">*</span></label>
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-12 mb-2"
                            ...
                            <textarea rows="3" class="form-control" id="note" name="note"></textarea>
                            @if ($errors->has('note'))
                                <span class="alert text-danger">
                                    {{ $errors->first('note') }}
                                </span>
...
@push('script')
    ...
    <script>
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.inventory.purchase.history.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
        ...
        function getSelectedUserData(id) {
            var url = '{{ route('admin.inventory.purchase.data', ':id') }}';
            $("#change").empty();
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
                  $('#received_amount').val(parseFloat(resp.amount) + parseFloat(resp.purchases.grand_total) - parseFloat(resp.amount));
                  $('#paying_amount').val(resp.amount);
                  $('#purchase_id').val(resp.id);
                  $('#date').val(resp.transaction_date);
                  $('#transaction_way').val(resp.transaction_account_type);
                  $('#change').append(parseFloat(resp.purchases.grand_total) - parseFloat(resp.amount));
                  ...
                  $(".fromClass").attr("action","{{ url('admin/inventory/purchase/update.purchase') }}/"+resp.id);
