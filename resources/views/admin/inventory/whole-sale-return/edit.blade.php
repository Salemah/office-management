...
@section('title', 'Whole Sale Return Edit')
...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        <ol class="breadcrumb my-0 ms-2">
            ...
            <li class="breadcrumb-item">
                 <a href="{{route('admin.inventory.whole-sale-return.index')}}">Whole Sale Return </a>
            </li>
            <li class="breadcrumb-item">
                 Edit
            </li>
        </ol>
        <a href="{{ route('admin.inventory.whole-sale-return.index') }}" class="btn btn-sm btn-dark">Back to list</a>
    </nav>
@endsection
...
@section('content')
    ...
    <form action="{{ route('admin.inventory.whole-sale-return.update',$return->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                                                ...
                                                <div >
                                                    <tbody id="purchase_product">
                                                        ...
                                                        <tr>
                                                            ...
                                                            $varId = '';
                                                            if($purchase->variant){
                                                                $varId =$purchase->variant->id;
                                                            }

                                                            @endphp
                                                            <td><input type="hidden" id="variant_id_{{$purchase->products->id}}{{$varId}}" value="{{$varId}}" class="form-control"   readonly > <input type="hidden" id="purchase_quantity_{{$purchase->products->id}}{{$varId}}" value="{{$purchase->stock_out}}" class="form-control"   readonly >{{$purchase->stock_out}}</td>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                                                </div>
                                            ...
                                            <table id="myTable" class="table table-hover order-list">
                                                ...
                                                <tbody id="product_data">
                                                    ...
                                                    <tr id="responsive-table-tr">
                                                        ...
                                                        <td><input type="number" id= "qty-{{$key+1}}" oninput="qtyCheck({{$key+1}})" name="qty[]" class="form-control" value="{{$inventory->whole_sale_return_qty}}"></td>
                                                        ...
                                                        <td><input type="hidden" name="variant[]"  id="variant-{{$key+1}}" value="@if($inventory->variant){{$inventory->variant->id}}@endif" class="form-control"   readonly ><button type="button"  class="jDeleteRow form-control btn btn-danger btn-icon waves-effect waves-light text-white" onclick="expensesRemove({{$key+1}})">
                                                        &times;
                                                        </button></td>
...
@push('script')
    <script>
            ...
            $.ajax({
                ...
                success: function ( data) {
                   // console.log(data[2]);
                    ...
                    if(data[2]){
                        var variantData =  data[2].value+ data[2].name+'-';
                    }
        ...
        function qtyCheck(x){
            ...
            if(qty>0){
                if(variant == variantQuantity){
                        var purchaseQuantity = $('#purchase_quantity_'+productId+variant).val();
        ...
        $('#product').select2({
            ajax: {
                url: '{{route('admin.inventory.whole-sale-return.product.search')}}',
                dataType: 'json',
                type: "POST",
