...
@section('title', 'Sale Payment History')
...
@push('script')
    ...
    <script>
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.inventory.sale.history.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
        ...
        function getSelectedUserData(id) {
            var url = '{{ route('admin.inventory.sale.data', ':id') }}';
            $("#change").empty();
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
                  $('#received_amount').val(parseFloat(resp.amount) + parseFloat(resp.sales.grand_total) - parseFloat(resp.amount));
                  $('#paying_amount').val(resp.amount);
                  $('#sale_id').val(resp.id);
                  $('#date').val(resp.transaction_date);
                  $('#transaction_way').val(resp.transaction_account_type);
                  $('#change').append(parseFloat(resp.sales.grand_total) - parseFloat(resp.sales.paid_amount));
                  ...
                  $(".fromClass").attr("action","{{ url('admin/inventory/sale/update.sale') }}/"+resp.id);
